<?php
    function getProducts($connection, $categoria){
        ...
        try{
            $consult_products = $connection->prepare("SELECT * FROM product WHERE category = '" . $categoria . "'");
        ...
        if($products_result == null){
            try{
                $consult_products = $connection->prepare("SELECT * FROM product WHERE $categoria = '1'");
                $consult_products->execute();
                $products_result = $consult_products->fetchAll(PDO::FETCH_ASSOC);
            }catch(PDOException $e){
                echo  "Error: " . $e->getMessage();
